...
#Santiago A. Bernal G. June 2025----- AMARU version 0.1
#This version was used for the publication of the article DOI:
#ADS: 
###############################################
#     █████╗ ███╗   ███╗ █████╗ ██████╗ ██╗   ██╗
#    ██╔══██╗████╗ ████║██╔══██╗██╔══██ ██║   ██║
...
"""
    ...
    E-mail: santiago.bernal@postgrado.uv.cl
...
class amaru:
    ...
    def __init__(self,file_dir, lwl=3800,upl=10400,islope=-3,fslope=0,str_op=0,ivar_lim=0.005,
        ...
        #Read the data
        self.file_reading(file_dir,lwl,upl,ivar_lim)
        cond_plot2=0
        #### Choose a model name
        ...
        self.model_constructor(model_name,self.stars_templates,self.gas_templates,self.Broad_templates,self.wind_templates,self.bc_templates,
                                 self.fe_templates,self.agn_templates)
        ...
        if plot_model_ini==1:
            self.plot_fit_model(file_dir,self.z,model_name,self.pp,self.lam_gal,self.nTemps,self.nLines1,
                                self.nLines2,self.nBlines,self.nwinds,
        ...
        if save_op==1: 
            self.fits_constructor(file_dir,self.z,self.scale_factor,model_name,self.pp,self.ivar,self.lam_gal,self.wdisp,
                              ...
                              self.nBc,self.nFe,self.slopes,self.nAgns,wts=2,fit_type=None,units=None)
        ...
        self.sig_one_reg,self.wo3_sig,self.dv_nr,self.case_winds,self.case_bels=dwt_deviations(self.devdf_dir,self.residuals,
                                                                                self.lam_gal,self.pp.sol,self.idx_o3,
                                                                                 self.idx_bels,self.pp.galaxy,self.pp.bestfit,
                                                                                 level=4,threshold_ps=1,plot_dev=1)
        ...
        if self.new_model=='no_new_model':
            ...
            if self.flag_vel_o3!=0:
                ...
                if o3gc!=None and np.absolute(self.new_inti_vel_narrow-self.pp.sol[o3gc][0])>50:
                ...
                #Same model but change in initial velocity of [OIII]d5007 if present    
                    self.model_constructor(model_name,self.stars_templates,self.gas_templates,
                                               self.Broad_templates,self.wind_templates,self.bc_templates,
                                     self.fe_templates,self.agn_templates,vel_o3=self.new_inti_vel_narrow,o3gc=o3gc)
                     ###############pPXF running ##############
                    start_time = timeit.default_timer() #start time to check
            ...
            self.model_constructor(model_name2,self.stars_templates,self.gas_templates,
                                           self.Broad_templates,self.wind_templates,self.bc_templates,
                                 self.fe_templates,self.agn_templates,vel_o3=new_init_vel_o3,o3gc=o3gc,bel_vel=bel_vel)
            ###############pPXF running ##############
            start_time = timeit.default_timer() #start time to check
        ...
        #-------
            ...
            if self.comparison_sc>0.5:
                ...
                self.model_constructor(model_name,self.stars_templates,self.gas_templates,self.Broad_templates,
                                                   self.wind_templates,self.bc_templates,
                                 self.fe_templates,self.agn_templates)
        ...
        if plot_best==True and cond_plot2==1:
            self.plot_fit_model(file_dir,self.z,self.best_model,self.best_pp,self.lam_gal,self.nTemps,self.nLines1,
                            self.nLines2,self.nBlines,self.nwinds,
        ...
        if save_op==1: 
            self.fits_constructor(file_dir,self.z,self.scale_factor,self.best_model,self.best_pp,self.ivar,self.lam_gal,self.wdisp,
                              ...
                              self.Bline_names, self.Bline_wave,self.nBlines,
                              self.wind_line_names, self.wind_line_wave,self.nwinds,
                              self.nBc,self.nFe,self.slopes,self.nAgns,fit_type=None,units=None)
            ...
            if nsim>0:
                ...
                for j in range(nsim):
                    ...
                    self.fits_constructor(file_dir,self.z,self.scale_factor,self.best_model,self.best_pp,self.ivar,self.lam_gal,self.wdisp,
                              ...
                              self.Bline_names, self.Bline_wave,self.nBlines,
                              self.wind_line_names, self.wind_line_wave,self.nwinds,
                              self.nBc,self.nFe,self.slopes,self.nAgns,fit_type='sim',units=None)
...
############################Read the SDSS fits file ###################################
    def file_reading(self, file_dir,lwl,upl,ivar_lim,zz=None):
        ...
        self.scale_factor =  np.median(flux)
    
        self.galaxy = flux/np.median(flux)   # Normalize spectrum to avoid numerical issues
        ...
        # noise = galaxy*0 + mivar#0.0166       # Assume constant noise per pixel here if not having 'ivar' or equivalent
...
########################## MODEL CONSTRUCTOR ##################################           
    ...
    def model_constructor(self,model_name,stars_templates,gas_templates,Broad_templates,wind_templates,bc_templates,
                    fe_templates,agn_templates,vel_o3=None,o3gc=None,bel_vel=None):
        ...
        start_wn = [vel-100, 500.] # (km/s), starting guess for [V,sigma] Gas narrow right
...
########################## COMPARISON ################################## 
    def comparison_two_fits(self,residuals_1,dof_1,residuals_2,dof_2,lam_gal,cp_ty=0):
        ...
        if cp_ty==2 or cp_ty==0:
            ...
            for my_lam in self.regions_comp:
                mask_reg=((lam_gal>my_lam[0])&(lam_gal<my_lam[1]))
...
########################## Plot fit results ##################################                 
    def plot_fit_model(self,file_dir,z,model_name,pp,lam_gal,nTemps,nLines1,nLines2,nBlines,nwinds,
...
########################## Save results ################################## 
    ...
    def fits_constructor(self,file_dir,z,scale_fact,model_name,pp,ivar,lam_gal,wdisp,st_names,nTemps,
                        ...
                        Bline_names, Bline_wave,nBlines, wind_line_names, wind_line_wave,nwinds,
                        nBc,nFe,slopes,nAgns,wts=0,fit_type=None,units=None):
       ...
       # print(nBlines2,nwinds2)    
        ...
        if model_name=='model_st_1' or model_name=='model_nst_1':
            my_comp_names=('Best-fit','stars','Gas_narrow_1','Gas_narrow_2','Gas_broad','Bal_Cont',
            'Fe_II','AGN','residuals','ivar','wave','wdisp')
            tcomp=Table([pp.bestfit,stars_f,np.array(comp_vals['gas_narrow_1f']),np.array(comp_vals['gas_narrow_2f']),
                    np.array(comp_vals['gas_broad_1f']),np.array(comp_vals['Bal_cont_f']),np.array(comp_vals['fe2_f']),
                    np.array(comp_vals['agn_f']),residuals,ivar,lam_gal,wdisp],names=my_comp_names)
    
            ###############
            #Kinematic results
            names1=('Component','velocity(Km/s)', 'velocity dispersion(Km/s)','h3','h4')
            names11=np.array(['stars','Gas_narrow1','Gas_narrow2','Gas_broad','Bal_Cont','Fe_II','AGN'])
        #Generate the vel array
            ...
            my_comp_names=('Best-fit','stars','Gas_narrow_1','Gas_narrow_2','Gas_broad','Gas_broad_2','Bal_Cont',
            'Fe_II','AGN','residuals','ivar','wave','wdisp')
            tcomp=Table([pp.bestfit,stars_f,np.array(comp_vals['gas_narrow_1f']),np.array(comp_vals['gas_narrow_2f']),
                    np.array(comp_vals['gas_broad_1f']),np.array(comp_vals['gas_broad_2f']),np.array(comp_vals['Bal_cont_f']),
                    np.array(comp_vals['fe2_f']),np.array(comp_vals['agn_f']),residuals,ivar,lam_gal,wdisp],names=my_comp_names)
    
            ###############
            #Kinematic results
            names1=('Component','velocity(Km/s)', 'velocity dispersion(Km/s)','h3','h4')
            names11=np.array(['stars','Gas_narrow1','Gas_narrow2','Gas_broad','Gas_broad_2','Bal_Cont','Fe_II','AGN'])
        #Generate the vel array
            vels=[]
            vels_disp=[]
            mnh3=[]
        ...
        #Now consider the different models
            ...
            my_comp_names=('Best-fit','stars','Gas_narrow_1','Gas_narrow_2','Gas_broad','winds','Bal_Cont',
            'Fe_II','AGN','residuals','ivar','wave','wdisp')
            tcomp=Table([pp.bestfit,stars_f,np.array(comp_vals['gas_narrow_1f']),np.array(comp_vals['gas_narrow_2f']),
                    np.array(comp_vals['gas_broad_1f']),np.array(comp_vals['winds_f']),np.array(comp_vals['Bal_cont_f'])
                    ,np.array(comp_vals['fe2_f']),np.array(comp_vals['agn_f']),residuals,ivar,lam_gal,wdisp],names=my_comp_names)
        ...
        #Generate the vel array
            ...
            my_comp_names=('Best-fit','stars','Gas_narrow_1','Gas_narrow_2','Gas_broad','Gas_broad_2','winds','Bal_Cont',
            'Fe_II','AGN','residuals','ivar','wave','wdisp')
            tcomp=Table([pp.bestfit,stars_f,np.array(comp_vals['gas_narrow_1f']),np.array(comp_vals['gas_narrow_2f']),
                    np.array(comp_vals['gas_broad_1f']),np.array(comp_vals['gas_broad_2f']),np.array(comp_vals['winds_f']),
                    np.array(comp_vals['Bal_cont_f']),np.array(comp_vals['fe2_f']),np.array(comp_vals['agn_f']),
                        residuals,ivar,lam_gal,wdisp],names=my_comp_names)
    
            ###############
            #Kinematic results
            names1=('Component','velocity(Km/s)', 'velocity dispersion(Km/s)','h3','h4')
            names11=np.array(['stars','Gas_narrow1','Gas_narrow2','Gas_broad','Gas_broad_2','winds','Bal_Cont','Fe_II','AGN'])
        #Generate the vel array
            vels=[]
            vels_disp=[]
            mnh3=[]
...
####################################### fluxes and EW
        ...
        F_cont=(pp.bestfit-np.array(comp_vals['gas_narrow_1f'])-np.array(comp_vals['gas_narrow_2f'])-
                    np.array(comp_vals['gas_broad_1f'])-Bels2-xwnds)*scale_fact
        #Narrow lines
        Nrr_flux=[]
        Nrr_EW=[]
        ...
        if nBlines2>0:
            ...
            for t in range(len(Bline_names)):
                L_flux=pp.matrix[:, nTemps+nLines1+nLines2+nBlines+t].dot(pp.weights[nTemps+nLines1+nLines2+nBlines+t])
                L_flux=L_flux*scale_fact
                I_flux=np.trapezoid(L_flux,x=lam_gal)
                Br_flux.append(I_flux)
            ##EW####
        ...
        if nwinds2>0:
            ...
            wind_flux=np.full((len(wind_line_names)),-999)
            EW_wd=np.full((len(wind_line_names)),-999)
